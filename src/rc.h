...
#include <string>
#include "config.h"
// Only used for the HVC patch, remove this
#include "hvc.h"
...
public:
    ...
    // Patch to use the old HVC with the new version
    RC(int process_id, int num_procs, HVC hvc)
    {
        ...
        vector<int> offset_vector = hvc.GetOffsets();
        vector<int> counter_vector = hvc.GetCounters();
        
        for(int i = 0; i < offset_vector.size(); i++)
        {
            offset_bitmap[i] = 1;
            SetOffsetAtIndex(i, offsets[i]);
        }

        for(auto i : counter_vector)
        {
            counters += i;
        }
    }
